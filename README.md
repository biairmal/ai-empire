...
**Status: V1.** A human creates a task. A worker implements it with Claude Code in an isolated git worktree and runs the tests. It pushes a branch and then stops at the merge gate. When a human approves, the worker merges the approved commit.
...
```sh
empire project create -slug demo -name "Demo" -repo git@github.com:me/demo.git \
    -stack go -autonomy medium -test "go test ./..."
empire task create -project demo -desc "GET /health returns {\"status\":\"ok\"}" add /health endpoint
empire task list
...
- **Context.** The control plane builds `.empire-context.md` from `knowledge/global`, `knowledge/stacks/<stack>`, and the task's `-doc` files under `knowledge/projects/<slug>/`, and nothing else. The file is git-excluded.
- **Isolation.** One worktree per task under `EMPIRE_WORKSPACES`. The agent runs with `--permission-mode acceptEdits` (it can edit files but not run shell commands), and `EMPIRE_*` and `DATABASE_URL` are stripped from its environment.
...
| Owner | Worker |
|-------|--------|
| `POST /projects`, `GET /projects[/{id}]` | `POST /workers/register` |
